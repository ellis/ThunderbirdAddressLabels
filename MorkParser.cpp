...
bool MorkParser::parse()
{
	...
	if (Result == false) {
		QString s = morkData_.mid(morkPos_ - 40, 80);
		qDebug() << "Error around:" << s;
	}
		...
		case '\\':
			{
				...
				if (NextChar == '\r')
				{
					// See if we have to eliminate a following newline
					NextChar = nextChar();
					if (NextChar != '\n')
						morkPos_--;
				}
...
bool MorkParser::parseTable()
{
	...
	if (Id < 0)
	{
		qDebug() << "DELETE TABLE: " << mork_[ abs( Scope ) ][ abs( Id ) ];
		mork_[ abs( Scope ) ].remove( -Id );
	}
...
inline void MorkParser::setCurrentRow( int TableScope, int TableId, int RowScope, int RowId )
{
	...
	if (RowId < 0)
	{
		qDebug() << "DELETE ROW " << QString::number(-RowId, 16) << ": " << ( mork_[ abs( TableScope ) ][ abs( TableId ) ][ abs( RowScope ) ][ abs( RowId ) ] );
		mork_[ abs( TableScope ) ][ abs( TableId ) ][ abs( RowScope ) ].remove( -RowId );
	}
...
bool MorkParser::parseRow( int TableId, int TableScope )
{
	...
	if ( Id < 0 || Id == 0x5EA )
	{
		qDebug() << "NEW ROW    " << QString::number(-Id, 16) << ": " << *currentCells_;
	}
